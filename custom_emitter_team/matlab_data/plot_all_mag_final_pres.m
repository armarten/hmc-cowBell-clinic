...
%     hold off;
    ...
    postergrey = [136 136 136]./255;
%     scatter(setpoints, instErrs * 100, 60, 'b','o','filled');
    errorbar(setpoints, instErrs .* 100, minErrs.* 100, maxErrs.* 100,'o','Color',postergrey, 'MarkerEdgeColor',cbGreen,'MarkerFaceColor',cbGreen, 'MarkerSize', 15, 'LineWidth', 2);
    hold on;
    yline([10, -10],"--","LineWidth",3)
    text(setpoints.*1.2,(instErrs .* 100),string(setpoints),'FontSize',14,'FontWeight','bold');
    xticks(setpoints)
    xtickformat('%.6g')
    xline(setpoints,":")
    ytickformat('%g%%')
