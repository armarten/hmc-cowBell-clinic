...
void loop() {
  ...
  String dataPrint = String(time_check_ms) + " , " + String(desiredFlowRate, 3) + " , " + String(currentBigFlowRate, 5) + " , " + String(currentSmallFlowRate, 5) + " , " + String(total_flow, 5) + " , " + String(controlEffort) + " , " + String(newgoalposition);
...
void controlBigMotor(float targetFlow) {
  ...
  while (abs(error) > 0.1 * bigTargetFlow) {
    ...
    // Crash check
    if (newPosition > 0 || newPosition < -30000) {
      Serial.println(newPosition);
